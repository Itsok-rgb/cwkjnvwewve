...
function launchConfetti() {
  ...
  for (let i = 0; i < pieces; i++) {
    ...
    const riseDuration = 3 + Math.random() * 1.8;
    const rotate = (Math.random() - 0.5) * 360;

    piece.style.left = left + "vw";
    piece.style.top = "110vh"; // start from bottom
    ...
    piece.animate(
      [
        ...
        {
          transform: `translate3d(${(Math.random() - 0.5) * 25}vw, -10vh, 0) rotate(${rotate}deg)`,
          opacity: 0.2
        }
      ],
      {
    ...
    setTimeout(() => piece.remove(), (riseDuration + delay + 0.5) * 1000);
  }
}
...
// Ensure init runs even though script is at the end of <body>
